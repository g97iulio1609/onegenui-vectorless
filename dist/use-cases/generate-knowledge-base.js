...
export class GenerateKnowledgeBaseUseCase {
    ...
    async execute(request) {
        ...
        const { extractEntities = true, extractRelations = true, extractQuotes = true, extractKeywords = true, extractCitations = true, generateSummaries = true, generateKeyInsights = true, } = options;
        const startTime = Date.now();
        // Generate hash for caching
        const hash = await this.generateHash(buffer);
        // Check cache
        const cached = await this.deps.cache.get(`kb:${hash}`);
        if (cached) {
        ...
        let tree = null;
        for await (const { event, entries, tocEndPage, } of agents.tocDetector.detectToc(pages)) {
            this.deps.sseEmitter.emit(event);
            if (entries) {
                for await (const { event: structEvent, node, } of agents.structureExtractor.extractStructure(pages, entries, tocEndPage || null)) {
        ...
        if (generateSummaries) {
            for await (const { event } of agents.summarizer.generateSummaries(tree, pages)) {
                this.deps.sseEmitter.emit(event);
            }
        }
        // Extract entities
        const entities = [];
        if (extractEntities) {
            for await (const { event, entity, } of agents.entityExtractor.extractEntities(pages, tree)) {
                this.deps.sseEmitter.emit(event);
                if (entity)
        ...
        const relations = [];
        if (extractRelations) {
            for await (const { event, relation, } of agents.relationExtractor.extractRelations(tree, entities)) {
                this.deps.sseEmitter.emit(event);
                if (relation)
        ...
        const quotes = [];
        if (extractQuotes) {
            for await (const { event, quote } of agents.quoteExtractor.extractQuotes(pages, tree)) {
                this.deps.sseEmitter.emit(event);
                if (quote)
                    quotes.push(quote);
            }
        ...
        const keywords = [];
        if (extractKeywords) {
            for await (const { event, keyword, } of agents.keywordExtractor.extractKeywords(pages, tree)) {
                this.deps.sseEmitter.emit(event);
                if (keyword)
        ...
        const citations = [];
        if (extractCitations) {
            for await (const { event, citation, } of agents.citationResolver.resolveCitations(pages, tree)) {
                this.deps.sseEmitter.emit(event);
                if (citation)
        ...
        // Calculate metrics
        const metrics = await agents.metricsCalculator.calculateMetrics(pages, tree);
        ...
        const knowledgeTree = this.convertToKnowledgeNode(tree, entities, quotes, citations);
    ...
    extractKeyInsights(tree, quotes, entities) {
        const insights = [];
    ...
    convertToKnowledgeNode(node, entities, quotes, citations) {
        ...
        return {
            ...
            rawText: node.text,
            children: (node.children || []).map((child) => this.convertToKnowledgeNode(child, entities, quotes, citations)),
        };
    }
}
//# sourceMappingURL=generate-knowledge-base.js.map