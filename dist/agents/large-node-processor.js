 ...
 */
function getNodeTokenCount(node, pages) {
 ...
 */
function isLargeNode(node, pages, options) {
    const start = node.pageStart ?? 1;
    const end = node.pageEnd ?? start;
 ...
 */
async function extractSubstructure(model, node, pages) {
    const start = node.pageStart ?? 1;
    const end = node.pageEnd ?? start;
    // Get pages for this node
    const nodePages = pages.filter((p) => p.pageNumber >= start && p.pageNumber <= end);
 ...
 */
export async function* processLargeNode(node, pages, options) {
 ...
 */
export async function* processLargeNodesInTree(tree, pages, options) {
    const timestamp = () => new Date().toISOString();
    const fullOptions = {
        ...DEFAULT_OPTIONS,
        ...options,
...
//# sourceMappingURL=large-node-processor.js.map