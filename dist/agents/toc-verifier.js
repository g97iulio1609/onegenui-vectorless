import { batchVerifyTitlesOnPages, batchVerifyTitlesAtStart, } from "./toc-verifier/index.js";
 ...
 */
export async function* verifyTreeStructure(model, tree, pages, options = {}) {
    ...
    const verifiableNodes = nodesToVerify.filter(({ node, depth }) => depth > 0 && node.pageStart !== undefined && node.pageStart > 0);
    // Sample if requested
    ...
    let startResults = new Map();
    if (options.checkPageStart) {
        const appearedRequests = verifyRequests.filter((_, i) => batchResults[i]?.appears);
...
//# sourceMappingURL=toc-verifier.js.map