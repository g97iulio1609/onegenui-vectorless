 ...
 */
export async function fixNodePageIndex(model, title, pages, searchRange) {
    const contextParts = [];
    for (const page of pages) {
        if (page.pageNumber >= searchRange.start &&
            page.pageNumber <= searchRange.end) {
            contextParts.push(`<physical_index_${page.pageNumber}>\n${page.content.slice(0, 2000)}\n<physical_index_${page.pageNumber}>`);
        }
    }
    const jsonExample = `{
 ...
 */
export async function* fixIncorrectNodes(model, tree, incorrectNodes, pages, options = {}) {
        ...
        for (const incorrectResult of currentIncorrect) {
            ...
            for (let i = nodeIndex - 1; i >= 0; i--) {
                ...
                if (prev) {
                    ...
                    if (!currentIncorrect.some((inc) => inc.nodeId === prev.id) &&
                        prevPageStart > 0) {
            ...
            for (let i = nodeIndex + 1; i < allNodes.length; i++) {
                ...
                if (next) {
                    ...
                    if (!currentIncorrect.some((inc) => inc.nodeId === next.id) &&
                        nextPageStart > 0) {
            ...
            if (fixResult && fixResult.confidence > 0.5) {
                ...
                if (options.verifyAfterFix) {
                    const pageContent = pages.find((p) => p.pageNumber === fixResult.pageNumber)?.content;
                    if (pageContent) {
                        const verification = await verifyTitleOnPage(model, node.title, pageContent);
...
//# sourceMappingURL=fix-nodes.js.map