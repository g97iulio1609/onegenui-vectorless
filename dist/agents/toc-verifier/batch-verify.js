 ...
 */
export async function batchVerifyTitlesOnPages(model, requests) {
 ...
 */
export async function batchVerifyTitlesAtStart(model, requests) {
    if (requests.length === 0)
        return new Map();
...
Return startsAtBeginning for each index.`;
    try {
        ...
        for (const r of result.object.results) {
            resultMap.set(requests[r.index]?.pageNumber ?? r.index, r.startsAtBeginning);
        }
        return resultMap;
    }
    catch (error) {
...
//# sourceMappingURL=batch-verify.js.map