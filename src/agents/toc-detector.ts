...
import { z } from "zod";
import { TocEntrySchema, type SSEEvent } from "../domain/schemas.js";
import type { Page, TocDetectorPort } from "../ports/index.js";
import {
  createSessionLogger,
...
export function createTocDetectorAgent(
  model: LanguageModel,
...
After analyzing, output ONLY the JSON object. Do not include any other text.`,
        ...
        tools: {
          analyzePage: tool({
            description:
              "Analyze a page to check for TOC content. Call this to examine pages not in the initial sample.",
            ...
            execute: async ({ pageNumber }) => {
              ...
              log.debug(
                "TOC",
                `Tool analyzePage called for page ${pageNumber}`,
              );
              return {
                pageNumber,
