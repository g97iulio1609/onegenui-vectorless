...
import type { Page, StructureExtractorPort } from "../ports/index.js";
import {
  createSessionLogger,
  type VectorlessLogger,
...
function convertToTreeNode(
  data: z.infer<typeof StructureOutputSchema>,
  totalPages: number,
): TreeNode {
  function convertSection(
    section: z.infer<typeof StructureOutputSchema>["sections"][0],
  ): TreeNode {
    return {
      id: randomUUID(),
      title: section.title,
...
export function createStructureExtractorAgent(
  model: LanguageModel,
  logger?: VectorlessLogger,
  ...
  return {
    async *extractStructure(
      ...
      const samplePageNumbers = new Set<number>();
...
After analyzing, output ONLY the JSON object. Do not include any other text.`,
        ...
        tools: {
          readPage: tool({
            description:
              "Read content of a specific page. Call this to examine pages not in the initial sample.",
            ...
            execute: async ({ pageNumber }) => {
              ...
              log.debug(
                "STRUCTURE",
                `Tool readPage called for page ${pageNumber}`,
              );
