  ...
  summaries: z.array(
    z.object({
      ...
      summary: z
        .string()
        .describe(
          "Comprehensive summary of the section content. Include key concepts, main arguments, and important details. Should be thorough but not verbose.",
        ),
    }),
  ),
...
export function createBatchSummarizerAgent(
  model: LanguageModel,
): SummarizerPort {
  /**
   * Recursively collect all nodes with their content
   */
  function collectNodesWithContent(
    tree: TreeNode,
    pages: Page[],
  ): NodeForSummary[] {
    const nodes: NodeForSummary[] = [];

    function traverse(node: TreeNode, index: number): void {
  ...
  async function generateBatchSummaries(
    nodes: NodeForSummary[],
    includeDocDescription: boolean = false,
  ): Promise<Map<string, string>> {
    const summaries = new Map<string, string>();

    if (nodes.length === 0) return summaries;
  ...
  return {
    async *generateSummaries(tree: TreeNode, pages: Page[]) {
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        const batchSummaries = await generateBatchSummaries(
          currentBatch,
          isLastBatch,
        );
...
${structureOverview}`,
        ...
        console.error(
          "[batch-summarizer] Description generation failed:",
          error,
        );
