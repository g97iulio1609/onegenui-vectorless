...
export class RelationExtractorAgent implements RelationExtractorPort {
  constructor(private model: LanguageModel) {}

  async *extractRelations(
    tree: TreeNode,
    entities: Entity[],
  ): AsyncGenerator<{ event: SSEEvent; relation?: Relation }> {
    yield {
      event: {
        type: "started",
...
Return ONLY the JSON object with relationships.`,
  ...
  ): { id: string; title: string; pageStart: number; pageEnd: number }[] {
    result.push({
