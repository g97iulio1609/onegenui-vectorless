...
const AgenticRetrievalOutputSchema = z.object({
  ...
  answer: z
    .string()
    .describe(
      "The final comprehensive answer with INLINE citation markers. Use [1], [2], [3] etc. to mark where each citation supports the text. Example: 'Il principio di precauzione [1] è fondamentale nel diritto ambientale [2].' Be detailed and thorough.",
    ),
  citations: z
    .array(
      z.object({
        ...
        excerpt: z
          .string()
          .describe(
            "The EXACT text quoted from the document. Must be a direct verbatim quote from the source, at least 50 characters.",
          ),
        nodeTitle: z.string().optional().describe("Section title if known"),
      }),
    )
    .describe(
      "Citations array. Each citation has an id (1, 2, 3...) matching the [N] markers in the answer text.",
    ),
});
...
function formatTreeForAgent(tree: TreeNode): string {
  const lines: string[] = [];
  ...
  function traverse(node: TreeNode, depth: number) {
    ...
    const summary = node.summary
      ...
      : "";
    lines.push(
      `${indent}[${node.id}] ${node.title} ${pageInfo}${hasText}${summary}`,
    );
...
After gathering information, output ONLY the JSON object.`;
  ...
  const getStructureTool = tool({
    description:
      "View the document's hierarchical structure (table of contents with summaries). " +
  ...
  const readSectionTool = tool({
    description:
      "Read the full content of a specific section by its node_id. " +
      "Use this after viewing the structure to get detailed content from a relevant section.",
    inputSchema: z.object({
      nodeId: z
        .string()
        .describe(
          "The node_id of the section to read (from get_structure output)",
        ),
    }),
        ...
        read_pages: tool({
          ...
          inputSchema: z.object({
            pageSpec: z
              .string()
              .describe(
                "Page numbers to read. Format: '5' for single page, '5-8' for range, '5,7,10' for specific pages",
              ),
          }),
          execute: async ({ pageSpec }: { pageSpec: string }) => {
            ...
            const contents: { pageNumber: number; content: string }[] = [];
