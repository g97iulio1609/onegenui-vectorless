...
export class QuestionAnswerAgent implements QuestionAnswerPort {
  constructor(private model: LanguageModel) {}

  async *answerQuestion(
    question: string,
    knowledgeBase: DocumentKnowledgeBase,
  ): AsyncGenerator<{ event: SSEEvent; answer?: Answer }> {
    yield {
      event: {
        type: "started",
...
Return ONLY the JSON object.`,
  ...
  private buildContext(kb: DocumentKnowledgeBase): {
    structure: string;
    entities: string;
    quotes: string;
  } {
    const structure = this.flattenTree(kb.tree)
      .map(
        (n) =>
          `${" ".repeat(n.level * 2)}- ${n.title} (p${n.pageStart}-${n.pageEnd}): ${n.summary}`,
      )
      .join("\n");
