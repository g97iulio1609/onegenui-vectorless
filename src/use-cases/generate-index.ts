...
import {
  verifyTreeStructure,
  fixIncorrectNodes,
  processLargeNodesInTree,
  addNodeText,
  removeNodeText,
  validateNodePageRanges,
  addPrefaceIfNeeded,
...
export class GenerateIndexUseCase {
  constructor(private deps: GenerateIndexUseCaseDeps) {}

  async execute(request: GenerateIndexRequest): Promise<GenerateIndexResult> {
    const {
      pdfParser,
      tocDetector,
      structureExtractor,
    ...
    const cached = await cache.get<DocumentIndex>(`index:${contentHash}`);
    if (cached) {
    ...
    for await (const result of tocDetector.detectToc(
      pages.slice(0, options.maxTocCheckPages ?? 20),
    )) {
      sseEmitter.emit(result.event);
      if (result.entries) tocEntries = result.entries;
      if (result.tocEndPage) tocEndPage = result.tocEndPage;
    }

    // 3. Extract structure
    let tree: TreeNode | undefined;
    for await (const result of structureExtractor.extractStructure(
      pages,
      tocEntries ?? null,
      tocEndPage ?? null,
    )) {
      sseEmitter.emit(result.event);
      if (result.node) tree = result.node;
    }
    ...
    if (options.verifyToc !== false) {
      let incorrectNodes: VerificationResult[] = [];
      ...
      if (options.fixIncorrectToc !== false && incorrectNodes.length > 0) {
        for await (const result of fixIncorrectNodes(
          model,
          tree,
          incorrectNodes,
          pages,
          { maxRetries: 3, verifyAfterFix: true },
        )) {
          sseEmitter.emit(result.event);
        }
      }
    ...
    const needsText =
      options.addSummaries !== false || options.addNodeText === true;
    if (needsText) {
  ...
  private hashContent(buffer: ArrayBuffer): string {
    return createHash("sha256").update(Buffer.from(buffer)).digest("hex");
  }

  private emitEvent(
    emitter: SSEEmitterPort,
    type: SSEEvent["type"],
  ...
  private applySummaries(node: TreeNode, summaries: Map<string, string>): void {
